...
tell application "iTerm"
    ...
    tell first session of current tab of current window
        write text "docker run --rm -it --network sconenet -v ~/dev/sconekv/deployment/configs/4-nodes.properties:/usr/src/sconekv/config.properties task3r/sconekv-node"
    end tell
    delay 1
