...
if [ $1 = "sconekv" ]; then
    docker run -t --network sconekv_sconenet us.gcr.io/scone-296713/tpcc java -jar TPCC.jar -s SconeKV -populate -r 0 -warmup 0 -n 0 -w $workers -warehouses $warehouses -nodes "$cluster_nodes"
elif [ $1 = "cassandra" ]; then
    docker run -t --network cassandra_cassnet us.gcr.io/scone-296713/tpcc java -jar TPCC.jar -s Cassandra -populate -r 0 -warmup 0 -n 0 -w $workers -warehouses $warehouses -nodes "$cluster_nodes"
elif [ $1 = "cockroach" ]; then
    docker run -t --network cockroach_roachnet us.gcr.io/scone-296713/tpcc java -jar TPCC.jar -s JDBC -populate -r 0 -warmup 0 -n 0 -w $workers -warehouses $warehouses -nodes "$cluster_nodes"
else
    echo "ERROR: Invalid system"
    exit
fi 
